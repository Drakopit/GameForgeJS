...
export class AdvancedDemoLevel extends Level {
    ...
    ResetLevel() {
        ...
        this.enemies.forEach(e => e.active = true);
    }
    ...
    OnUpdate(dt) {
        ...
        // 2. Morte por encostar no inimigo
        this.enemies.forEach(enemy => {
            if (enemy.active && Collide2D.isCollidingAABB(this.player, enemy)) {
                this.ResetLevel();
            }
        });
        ...
        // Checa Colisões: Tiro vs Inimigo
        this.bulletPool.pool.forEach(bullet => {
            if (bullet.active) {
                this.enemies.forEach(enemy => {
                    if (enemy.active && Collide2D.isCollidingAABB(bullet, enemy)) {
                        enemy.active = false;
                        bullet.active = false; 
                    }
                });
